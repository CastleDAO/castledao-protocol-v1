...
contract Crafting is PausableUpgradeable, ReentrancyGuardUpgradeable, ManagerModifierUpgradeable {
    ...
    function addRecipe(
        ...
        require(inputItems.length == inputAmounts.length, "Input items and amounts length mismatch");

        recipes[recipeId] = Recipe(inputItems, inputAmounts, magicCost, rubyCost, outputItem, successChance);
    }
    ...
    function craft(uint256 recipeId) external whenNotPaused nonReentrant {
        ...
        for (uint256 i = 0; i < recipe.inputItems.length; i++) {
            itemsContract.burn(msg.sender, recipe.inputItems[i], recipe.inputAmounts[i]);
        }

        // Transfer MAGIC and RUBY tokens
        if (recipe.magicCost > 0) {
            magicToken.transferFrom(msg.sender, address(this), recipe.magicCost);
        }
        ...
        uint256 random = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender, recipeId))) % 100;
